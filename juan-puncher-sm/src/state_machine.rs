...
impl StateMachineNode for SelectedState {
    fn process_packet(
        ...
        match received_lane_status {
            LaneStatus::Connecting | LaneStatus::Establishing => Ok(TransitionRequest::Remain),
            LaneStatus::Selected if is_server && !self.sent => Err(BlockReason::UnexpectedTransition),
...
impl StateMachineNode for LaneState {
    fn process_packet(
        ...
        match self {
            Self::Connecting(t) => t.process_packet(is_server, has_selected, received_lane_status),
            Self::Establishing(t) => t.process_packet(is_server, has_selected, received_lane_status),
            Self::Selected(t) => t.process_packet(is_server, has_selected, received_lane_status),
            Self::Blocked(_) | Self::Closed => Ok(TransitionRequest::Remain),
        }
    }
